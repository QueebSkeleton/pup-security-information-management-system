...
public class InspectionTableModel extends AbstractTableModel {
	 ...
	 * Since it has a backing repository as its source of data of inspections,
	...
	public Object getValueAt(int rowIndex, int columnIndex) {
		...
		Inspection inspection = cache.get(rowIndex);
		...
		// Second Column - Date
		case 1:
			return inspection.getDate();
			
		// Third Column - Building - Floor
		case 2:
			return inspection.getBuilding().toString() + " - " + inspection.getFloorNumber();
			
		// Fourth Column - Room Numbers
		case 3:
			return inspection.getRoomNumbers();
			
		// Fifth Column - Issue Count
		case 4:
			return inspection.getIssues().size();
			
		// Sixth Column - General Condition
		case 5:
			return inspection.getGeneralCondition().toString();
	...
	public void update() {
		...
		// and execute it.
		new SwingWorker<List<Inspection>, Void>() {
			@Override
			protected List<Inspection> doInBackground() throws Exception {
				// Fetch all data from the repository
				return inspectionManagementPanel.inspectionRepository.getAll();
			}
			@Override
			protected void done() {
				try {
